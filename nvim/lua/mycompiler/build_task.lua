local build_task = require('mycompiler.config')
...
function BuildTask:get_compiler()
    return vim.bo.filetype == "cpp" and build_task.config.default_cpp_compiler or build_task.config.default_c_compiler
end
...
function BuildTask:get_compile_flags(filename)
    ...
    return vim.bo.filetype == "cpp" and build_task.config.default_cpp_flags or build_task.config.default_c_flags
end
...
function BuildTask:compile()
    ...
    local outfile = build_task.config.output_directory .. vim.fn.expand("%:t:r")
    local infile = vim.api.nvim_buf_get_name(0)
    local cmd_compile = build_task.config.compile_command
...
function BuildTask:run()
    local outfile = build_task.config.output_directory .. vim.fn.expand("%:t:r")
    local hash = BuildTask:get_buffer_hash()

    if vim.b.last_compiled_hash ~= hash then
        local success = self:compile()
...
function BuildTask:show_assembly()
    ...
    local cmd_assemble = build_task.config.assemble_command
        or string.format("%s %s -S -o %s %s", compiler, flags, asm_file, infile)
    local diagnostics = vim.diagnostic.get(0, { severity = { vim.diagnostic.severity.ERROR } })
    local hash = self:get_buffer_hash()

    if vim.b.last_assembled_hash ~= hash then
        if vim.tbl_isempty(diagnostics) then
            vim.cmd("silent! write")
            vim.fn.system(cmd_assemble)
            vim.b.last_assembled_hash = hash
        else
            self:goto_first_diagnostic(diagnostics)
            return
        end
